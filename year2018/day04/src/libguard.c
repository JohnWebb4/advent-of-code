...
int libguard_get_strategy_1_best_guard(struct libguard_event *const *events, size_t events_length)
{
    ...
    int times_asleep_minute_in_a_day[MINUTES_IN_A_DAY] = {0};
...
int libguard_get_strategy_2_best_guard(struct libguard_event *const *events, size_t events_length)
{
    size_t num_sleep_times = 0;
    struct libguard_sleep_times *sleep_times = calloc(events_length / 2 + 1, sizeof(*sleep_times));
    ...
    int minute_most_often_asleep = 0;
    int times_most_often_asleep = 0;
    int best_guard_id = -1;
    for (size_t guard_i = 0; guard_i < num_guards; guard_i++)
    {
        int recurring_minutes_asleep[MINUTES_IN_A_DAY] = {0};

        for (size_t sleep_time_i = 0; sleep_time_i < num_sleep_times; sleep_time_i++)
        {
            if (sleep_times[sleep_time_i].guard_id == guard_ids[guard_i])
            {
                for (int minute_i = sleep_times[sleep_time_i].minute_asleep; minute_i < sleep_times[sleep_time_i].minute_awake; minute_i++)
                {
                    recurring_minutes_asleep[minute_i] += 1;

                    if (times_most_often_asleep < recurring_minutes_asleep[minute_i])
                    {
                        minute_most_often_asleep = minute_i;
                        times_most_often_asleep = recurring_minutes_asleep[minute_i];
                        best_guard_id = guard_ids[guard_i];
                    }
                }
            }
        }
    ...
    return best_guard_id * minute_most_often_asleep;
}
